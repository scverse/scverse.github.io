...
We'll highlight some of the major changes here but be sure to check out [the changelog](https://anndata.scverse.org/en/latest/release-notes/) for the full list of changes, including breaking changes.
...
But treating `X` as just another layer moves us towards encouraging our users to be explicit about what `X` really is.
...
We introduced support for the zarr v3 file-format as well as the v3 `zarr-python` package in [0.12](/blog/2025-anndata-012/).
...
0.13 introduces the new [`anndata.acc`](https://anndata.scverse.org/en/latest/accessors.html) module for referring to a vector or array *by location* without binding it to a specific object.

The use cases for this feature abound: plotting functionality, validation, or arguments for things like "the `louvain` column" or "the first PCA component".
