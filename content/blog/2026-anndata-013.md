...
In the future, we aim for `scanpy`'s API to look more like:

```python
sc.pl.scatter(adata, A.X[:, ["PSAP", "C1QA"]], color=A.obs["leiden"])
```

which gives a scatter plot with the two genes `PSAP` and `C1QA` plotted against each other, colored by leiden category.
